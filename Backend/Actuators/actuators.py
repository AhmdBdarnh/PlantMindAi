...
class GH_Actuators:     
    def __init__(self, esp32_i2c_address: int, i2c_bus: busio.I2C = busio.I2C(board.SCL, board.SDA), frame_endianes: str = 'big'):
        ...
        self.__heater_mqtt_dc_value = 0
        self.__heater_fan_mqtt_dc_value = 0
        self.__fan_mqtt_dc_value = 0
    ...
    def get_mqtt_dc_value_heater(self) -> int:
        return self.__heater_mqtt_dc_value
    
    def set_mqtt_dc_value_fan(self, mqtt_dc_value: int):
        self.__fan_mqtt_dc_value = mqtt_dc_value
    
    def get_mqtt_dc_value_fan(self) -> int:
        return self.__fan_mqtt_dc_value
    
    def set_mqtt_dc_value_light_strip(self, mqtt_dc_value: int):
        self.__light_strip_1_mqtt_dc_value = mqtt_dc_value

    def get_mqtt_dc_value_light_strip(self) -> int:
        return self.__light_strip_1_mqtt_dc_value
    
    def stop_all_actuators(self) -> bool:
        MAX_RETRIES = 3
        all_ok = True
        for name, fn in [
            ("water_pump",       lambda: self.set_water_pump_duty_cycle(0)),
            ("fertilizer_pump",  lambda: self.set_fertilizer_pump_duty_cycle(0)),
            ("heater",           lambda: self.set_heater_duty_cycle(0)),
            ("heater_fan",       lambda: self.set_heater_fan_duty_cycle(0)),
            ("fan",              lambda: self.set_fan_duty_cycle(0)),
            ("light_strip_1",    lambda: self.set_light_strip_1_duty_cycle(0)),
            ("light_strip_2",    lambda: self.set_light_strip_2_duty_cycle(0)),
        ]:
            ok = False
            for _ in range(MAX_RETRIES):
                ...
                time.sleep(0.05)
            if not ok:
                _CUSTOM_PRINT_FUNC(f"[Actuators] Warning: could not stop {name} after {MAX_RETRIES} retries")
                all_ok = False
        _CUSTOM_PRINT_FUNC("All actuators stopped." if all_ok else "All actuators stop attempted (some failed).")
        return all_ok
...
class GH_Actuators:     
    ...
    def setup_light_esp32(self, pin: int, channel: int, timer_src: int, frequency: int, duty_cycle: int) -> bool:
        ...
        try:
            return self.__send_init_request(pin, channel, timer_src, frequency, duty_cycle, "light")
        except Exception as e:
            # print(f"Error setting up light: {e}")
            return False
    
    def set_light_duty_cycle(self, duty_cycle: int) -> bool:
        try:
            return self.__send_duty_cycle_update_request(duty_cycle, self.__light_pin, self.__light_channel, "light")
        except Exception as e:
    ...
    def get_light_duty_cycle(self) -> int:
        try:
